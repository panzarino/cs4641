...
def linreg_closed_form(train_X, train_Y):
	...
	'''
	Theta = numpy.linalg.inv(train_X.transpose().dot(train_X)).dot(train_X.transpose()).dot(train_Y)
	return Theta
...
def loss(Theta, train_X, train_Y):
	...
	'''
	rv = numpy.sum(numpy.square(train_X.dot(Theta) - train_Y)) / (2 * train_X.shape[0])
	return rv
...
def linreg_grad_desc(initial_Theta, train_X, train_Y, alpha=0.1, num_iters=500, print_iters=True):
	...
	for k in range(1,num_iters+1):
		...
		cur_Theta = cur_Theta - alpha * ((train_X.transpose().dot(train_X.dot(cur_Theta)) - train_X.transpose().dot(train_Y)) / train_X.shape[0])
	return step_history
...
def random_fourier_features(train_X, train_Y, num_fourier_features=100, alpha=0.1, num_iters=500, print_iters=False):
	...
	Omega = numpy.random.normal(size = [numpy.shape(train_X)[1], num_fourier_features])
	B = numpy.random.random(size = num_fourier_features) * numpy.pi * 2
